<template lang="pug">
#deleteUser(@click="emits('close')")
    .wrap(@click.stop)
        .title Delete User
        p This action will delete {{ props.checkedUsers.length }} user(s) account and all the user's data, This action cannot be undone.
        .buttonWrap 
            template(v-if="promiseRunning")
                img.loading(src="@/assets/img/loading.png")
            template(v-else)
                button.cancel(@click="emits('close')") Cancel
                button.disable(@click="deleteUsers") Delete
</template>
