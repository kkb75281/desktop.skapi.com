...
#blockUser(@click="emits('close')")
    .wrap(@click.stop)
        .title Unblock User
        p This action will unblock {{ props.checkedUsers.length }} user(s) from your service. The user will have access to your service.
        .buttonWrap 
            template(v-if="promiseRunning")
                img.loading(src="@/assets/img/loading.png")
            template(v-else)
                button.cancel(@click="emits('close')") Cancel
                button.disable(@click="blockUsers") Unblock
</template>
